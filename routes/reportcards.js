...
// ── GET /api/reportcards/list — for Reports tab ───────────────
router.get('/list', async (req, res) => {
  const { school, class_name, student_id, term, academic_year } = req.query;
  const user = req.session.user;
  ...
  // Auth: teachers see only their classes
  if (user.role !== 'admin') {
    rows = rows.filter(r => user.classes.some(c =>
      c.school === r.cloud_students?.school && c.class_name === r.cloud_students?.class_name
    ));
  }

  // Fetch marks for each card
  const result = [];
  for (const card of rows) {
    ...
    result.push({
      ...card,
      student_name: card.cloud_students?.name,
      roll_no: card.cloud_students?.roll_no,
      class_name: card.cloud_students?.class_name,
      division: card.cloud_students?.division,
      school: card.cloud_students?.school,
  ...
  result.sort((a, b) => (parseInt(a.roll_no) || 0) - (parseInt(b.roll_no) || 0));
  res.json(result);
});
